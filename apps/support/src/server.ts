...
async function aiReply(message) {
  if (!OPENAI_API_KEY) {
    if (/ticket|issue|bug|problem/i.test(message))
      return 'I can open a support ticket for you. Share a short title + details; I’ll log it and notify the team.';
    if (/billing|invoice|charge/i.test(message))
      return 'For billing, include order ID / email; I’ll file a secure ticket.';
  ...
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'TokPulse Support: concise, safe, helpful. If ticket requested, gather title/details/contact then suggest creating one.',
          },
    ...
    return 'Thanks — I’ll log this and follow up.';
  }
}
function ensureData() {
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && parsed.pathname === '/healthz')
    return json(res, 200, { ok: true, service: 'support' });
