...
const REPORT_PATH =
  process.env.REPORT_PATH || path.join(ROOT, '..', 'packages', 'data', 'last-report.json');
...
const server = http.createServer((req, res) => {
  ...
  if (parsed.pathname === '/healthz')
    return send(res, 200, JSON.stringify({ ok: true, service: 'dashboard' }), 'application/json');
  ...
  if (parsed.pathname === '/api/license') {
    try {
      ...
      return send(
        res,
        200,
        JSON.stringify({ license: 'free', pro: false, features: ['dashboard', 'support'] }),
        'application/json',
      );
    }
  }
