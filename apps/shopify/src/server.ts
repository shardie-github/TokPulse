...
function loadVault() {
  try {
    const p = JSON.parse(fs.readFileSync(VAULT_FILE, 'utf-8'));
    return decryptJSON(p);
  } catch {
    return { shops: {} };
  }
}
function saveVault(v) {
  const enc = encryptJSON(v);
  fs.mkdirSync(path.dirname(VAULT_FILE), { recursive: true });
  fs.writeFileSync(VAULT_FILE, JSON.stringify(enc));
}
...
function rememberWebhook(id) {
  ...
  let db = {};
  try {
    db = JSON.parse(fs.readFileSync(REPLAY_DB, 'utf-8'));
  } catch {}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && parsed.pathname === '/api/shopify/callback') {
    ...
    const stateCookie = (req.headers.cookie || '')
      .split(';')
      .map((s) => s.trim())
      .find((s) => s.startsWith('shopify_state='))
      ?.split('=')[1];
    ...
    try {
      ...
      res.writeHead(302, { Location: APP_URL + '/?shopify=ok' });
      return res.end();
    } catch (e) {
  ...
  if (req.method === 'POST' && parsed.pathname === '/api/shopify/webhook') {
    ...
    req.on('end', () => {
      ...
      if (!ok || !rememberWebhook(String(id || ''))) {
        log('shopify.log', { event: 'webhook-reject', id });
      ...
      try {
        ...
        log('shopify.log', { event: 'webhook', topic, shop });
      } catch {}
    });
    return;
  }
