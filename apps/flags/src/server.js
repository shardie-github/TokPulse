...
function sec(res){ res.setHeader('X-Content-Type-Options','nosniff'); res.setHeader('Access-Control-Allow-Origin',ORIGIN) }
...
const server=http.createServer((req,res)=>{
  ...
  if(p.pathname==='/api/flags'){ try{ const txt=fs.readFileSync(FILE,'utf-8'); return json(res,200,JSON.parse(txt)) }catch{ return json(res,200,{}) } }
