...
function okLicense(expires = null) {
  const lic = {
    license: 'pro',
    pro: true,
    features: ['dashboard', 'support', 'insights', 'export'],
    expires,
  };
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && parsed.pathname === '/healthz')
    return json(res, 200, { ok: true, service: 'billing', sandbox: SANDBOX });
  ...
  if (req.method === 'POST' && parsed.pathname === '/api/billing/checkout' && SANDBOX) {
    ...
    req.on('end', () => {
      try {
        ...
        const base = b.base_url || process.env.PUBLIC_BASE_URL || 'http://localhost:4173';
        json(res, 200, { url: `${base}/?upgrade=success&sandbox=1` });
      } catch {
        json(res, 200, { url: 'http://localhost:4173/?upgrade=success&sandbox=1' });
      }
    });
    return;
  }
  ...
  if (req.method === 'POST' && parsed.pathname === '/api/billing/checkout') {
    ...
    req.on('end', async () => {
      try {
        ...
        json(res, 500, { error: 'checkout-failed', detail: String(e?.message || e) });
      }
    });
    return;
  }
            ...
            stripe.subscriptions
              ...
              .then((s) => {
                if (s?.current_period_end)
                  okLicense(new Date(s.current_period_end * 1000).toISOString());
              })
              .catch(() => okLicense(null));
          } else {
            const s = event.data.object;
            if (s?.current_period_end)
              expires = new Date(s.current_period_end * 1000).toISOString();
            okLicense(expires);
          }
        } catch {
          okLicense(null);
