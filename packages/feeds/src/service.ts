...
export class FeedsService {
  ...
  // Feed Generation
  async generateFeed(storeId: string, feedType: FeedType, forceRegenerate: boolean = false): Promise<FeedUrl> {
    ...
    if (!forceRegenerate && feedConfig.lastGenerated) {
      ...
      if (hoursSinceLastGeneration < 1) { // Don't regenerate if less than 1 hour old
  ...
  private async saveFeed(storeId: string, feedType: FeedType, content: string, mimeType: string): Promise<FeedUrl> {
  ...
  async scheduleFeedGeneration(): Promise<void> {
    ...
    for (const feedConfig of feedConfigs) {
      try {
        ...
        console.error(`❌ Failed to generate ${feedConfig.type} feed for store ${feedConfig.storeId}:`, error)
  ...
  // Feed Validation
  async validateFeed(storeId: string, feedType: FeedType): Promise<{ valid: boolean; errors: string[] }> {
    try {
      const feedConfig = await this.db.feedConfig.findFirst({
        where: { 
          storeId, 
  ...
  async getFeedStats(storeId: string): Promise<{
    ...
    const lastGenerated = activeFeeds.length > 0 
      ? new Date(Math.max(...activeFeeds.map(feed => feed.lastGenerated?.getTime() || 0)))
      : null
    const nextGeneration = activeFeeds.length > 0
      ? new Date(Math.min(...activeFeeds.map(feed => feed.nextGeneration?.getTime() || Infinity)))
      : null
