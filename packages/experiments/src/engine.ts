...
import { z } from 'zod'
import { experimentExposureTotal, experimentAssignmentTotal, experimentGuardrailBreachTotal } from '@tokpulse/telemetry'
...
export class ExperimentEngine {
   ...
   */
  private generateAssignment(experiment: ExperimentConfig, subjectKey: string): AssignmentResult | null {
    // Check allocation
   ...
   */
  async checkGuardrails(experimentKey: string, metric: string, value: number, threshold: number): Promise<boolean> {
    const experiment = this.experiments.get(experimentKey)
