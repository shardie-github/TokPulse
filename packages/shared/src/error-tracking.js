export class ErrorTracker {
    ...
    async captureWarning(message, context = {}, tags = {}) {
        const errorId = this.generateErrorId();
        const timestamp = new Date().toISOString();
        const errorEvent = {
            id: errorId,
    ...
    async captureInfo(message, context = {}, tags = {}) {
        const errorId = this.generateErrorId();
        const timestamp = new Date().toISOString();
        const errorEvent = {
            id: errorId,
    ...
    determineErrorLevel(error) {
        ...
        if (highErrors.includes(error.name))
            return 'high';
        if (mediumErrors.includes(error.name))
            return 'medium';
        return 'low';
    }
    async flush() {
    ...
    async assignError(errorId, assignedTo) {
        // In a real implementation, this would update the database
    ...
    async monitorApiRequest(method, path, statusCode, duration, context = {}) {
...
export function createErrorTrackingMiddleware(errorTracker) {
    return (req, res, next) => {
        const startTime = Date.now();
        ...
        res.json = function (data) {
            const duration = Date.now() - startTime;
            // Track API request
            errorTracker.monitorApiRequest(req.method, req.path, res.statusCode, duration, {
                requestId,
                method: req.method,
                url: req.url,
...
export const errorTracker = new ErrorTracker({}, // Will be replaced with actual DB instance
{
