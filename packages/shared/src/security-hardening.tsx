...
export const SecurityHardeningSchema = z.object({
  ...
  cors: z.object({
    ...
    allowedHeaders: z
      .array(z.string())
      .default(['Content-Type', 'Authorization', 'X-Requested-With', 'X-CSRF-Token']),
  ...
  logging: z.object({
    ...
    excludeFields: z
      .array(z.string())
      .default(['password', 'token', 'secret', 'key', 'authorization', 'cookie']),
  }),
...
export class SecurityHardening {
  ...
  validateRequest(req: any): { valid: boolean; errors: string[] } {
    ...
    if (this.config.validation.validateContentType) {
      ...
      if (
        contentType &&
        !this.config.validation.allowedContentTypes.includes(contentType.split(';')[0])
      ) {
  ...
  validateWebhookSignature(payload: string, signature: string, secret: string): boolean {
    ...
    const expectedSignature = createHash('sha256').update(payload).update(secret).digest('hex');
...
export const defaultSecurityHardeningConfig: SecurityHardeningConfig = {
  ...
  logging: {
    ...
    format: 'json',
    sanitize: true,
    excludeFields: ['password', 'token', 'secret', 'key', 'authorization', 'cookie'],
  },
  database: {
    ssl: true,
    connectionLimit: 10,
