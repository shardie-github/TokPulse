...
import { ShopifyBillingService } from './shopify'
import { BillingWebhookHandler } from './webhooks'
import { CreateSubscriptionRequest, UpdateSubscriptionRequest, RecordUsageRequest } from './types'
...
export function createBillingRoutes(config: BillingRoutesConfig): Router {
  ...
  router.post('/shopify/checkout', async (req: Request, res: Response) => {
    try {
      ...
      const { confirmationUrl, chargeId } = await config.shopifyBillingService.createRecurringApplicationCharge(
        shopDomain,
        accessToken,
        plan,
        trialDays
