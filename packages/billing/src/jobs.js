export class BillingJobs {
    ...
    async processTrialExpirations() {
        ...
        for (const subscription of expiringTrials) {
            ...
            if (daysLeft <= 3 && daysLeft > 0) {
                try {
                    await this.config.emailService.sendTrialEndingNotification(subscription.organizationId, daysLeft);
        ...
        console.log(`Processed ${expiringTrials.length} expiring trials and ${expiredTrials.length} expired trials`);
    }
    async processPaymentFailures() {
    ...
    async processCancelledSubscriptions() {
        ...
        for (const subscription of recentlyCancelled) {
            try {
                await this.config.emailService.sendSubscriptionCancelledNotification(subscription.organizationId);
